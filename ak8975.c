...
{
    /* 補正値を使って補正済みHを計算 */
    unit->adj_x = ( ( ( unit->coeff_x - 128.0 ) * 0.5 ) / 128 + 1 ) * unit->x;
    unit->adj_y = ( ( ( unit->coeff_y - 128.0 ) * 0.5 ) / 128 + 1 ) * unit->y;
    unit->adj_z = ( ( ( unit->coeff_z - 128.0 ) * 0.5 ) / 128 + 1 ) * unit->z;
}
...
char ak8975_self_test( AK8975Unit *unit )
{
    ...
    if ( !i2c_write_register( unit->address, 0x0C, &data, 1, I2CPolling ) ) {
        return 0;
    }
    ...
    /* 完了待ち */
    while ( !ak8975_data_ready( unit ) );
    
    /* 読み込み・変換 */
    ak8975_read( unit );
    ak8975_calc_adjusted_h( unit );
    ...
    /* 確認 */
    if ( unit->adj_x > 100 || unit->adj_x < -100 ) {
        return 0;
    }
    
    if ( unit->adj_y > 100 || unit->adj_y < -100 ) {
        return 0;
    }
    
    if ( unit->adj_z > -300 || unit->adj_z < -1000 ) {
        return 0;
    }
